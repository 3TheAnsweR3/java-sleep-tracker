...
public class MinimumDurationAnalysisTest {

    @Test
    @DisplayName("Должен возвращать ноль для пустого списка сессий")
    void shouldReturnZeroForEmptySessionsList() {
        MinimumDurationAnalysis minimumDurationAnalysis = new MinimumDurationAnalysis();
        List<SleepingSession> list = new ArrayList<>();
        SleepAnalysisResult result = minimumDurationAnalysis.apply(list);

        assertEquals(0L, result.getValue());
    }

    @Test
    @DisplayName("Должен возвращать минимальную продолжительность сессии")
    void shouldReturnMinimumSessionDuration() {
        MinimumDurationAnalysis minimumDurationAnalysis = new MinimumDurationAnalysis();
